function [] = sortResampleImages( ip )
...
pw = [experimentFolder filesep ip.listAcquisitions{ip.iAcquisitions,2} filesep 'readImages'];
cd (char(pw))
clear dir
...
% Find number of max time points
for ii = 1:length(allFiles)
    if contains(allFiles(ii),'t01') == 1
        tMax = 99;
        break
    elseif ii == length(allFiles)
        tMax = 999;
    end
end
...
for iTimes = 1:nTimes
    for iPlanes = 1:zDim
        for iChannels = 2:nChannels+1
            ...
            source = [experimentFolder filesep char(ip.listAcquisitions{ip.iAcquisitions,2}) filesep 'readImages' filesep filename];
            destination = [experimentFolder filesep char(ip.listAcquisitions{ip.iAcquisitions,2}) filesep 'sortedImages' filesep char(tsChannels{iChannels-1}) filesep filename];
...
for iChannels = 2:nChannels+1
    readpw = [experimentFolder filesep ip.listAcquisitions{ip.iAcquisitions,2} filesep 'sortedImages' filesep tsChannels{iChannels-1}];
    cStr = 'c%d';
    parfor iTime = 1:nTimes
        ...
        savepw = [experimentFolder filesep ip.listAcquisitions{ip.iAcquisitions,2} filesep 'resampledImages' filesep tsChannels{iChannels-1}]; %#ok<*PFBNS>
        cd (char(savepw))
