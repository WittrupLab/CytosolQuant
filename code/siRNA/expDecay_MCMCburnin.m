function theta = expDecay_MCMCburnin(ydata, xdata, sim)
%%
%   estimation parameters in the dubbel expontial decay function
...
obj = struct;
obj.Sigma_hat = zeros(6,6);
obj.theta_hat = zeros(6,1);
obj.Sigma     = eye(6); 
...
sigma = sqrt(var((ydata - expDecay(xdata,theta))));
obj.logpi = @(theta) - sum((ydata - expDecay(xdata,theta)).^2)/sigma^2 - sum(theta.^2)/5^2;
...
for i= 1:sim
 ...
 beta   = sum((ydata - expDecay(xdata,theta)).^2)/2;
